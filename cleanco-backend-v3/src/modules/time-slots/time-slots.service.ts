...
export class TimeSlotsService {
  ...
  /**
   * Get available time slots for admin (no 6-hour restriction, but hide past slots)
   */
  async getAvailableSlotsForAdmin(query: AvailableSlotsQueryDto) {
    const { date: dateString, requiredCleaners: overrideCleaners } = query;

    // Use provided requiredCleaners if specified, otherwise calculate
    const requiredCleaners = overrideCleaners ?? this.calculateRequiredCleaners(query);
    ...
    // For same-day bookings (admin): only hide slots that have ALREADY PASSED
    // No 6-hour buffer like regular users
    let finalSlots = slotsWithAvailability;
    if (isToday) {
      // Get current time in Maldives timezone as hours and minutes
      const currentTimeInMaldives = DateUtils.formatInMaldives(now, 'HH:mm');
      ...
      finalSlots = slotsWithAvailability.map((slot) => {
        const [slotHours, slotMinutes] = slot.startTime.split(':').map(Number);
        const slotTotalMinutes = slotHours * 60 + slotMinutes;
        // Hide if slot has already passed (started)
        const hasPassed = slotTotalMinutes <= currentTotalMinutes;

        return {
          ...slot,
          isAvailable: hasPassed ? false : slot.isAvailable,
        };
      });
    }
  ...
  /**
   * Get subscription availability for admin (no 6-hour restriction on first booking)
   */
  async getSubscriptionAvailabilityForAdmin(query: SubscriptionAvailabilityQueryDto) {
    const { startDate: startDateString, dayOfWeek } = query;
    ...
    // For admin: only hide slots that have PASSED on the first booking date (no 6-hour buffer)
    const firstBookingDate = datesToCheck[0];
    const firstBookingDateIsToday = format(firstBookingDate, 'yyyy-MM-dd') === format(today, 'yyyy-MM-dd');
